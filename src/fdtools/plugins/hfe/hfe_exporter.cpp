...
bool fdt_hfe_image_export(FdtFileImage* img, FdtError* err)
{
  ...
  for (size_t t = 0; t < number_of_track; t++) {
    ...
    size_t track_block_count = size_t(ceil((double)track_offsets[t].track_len / (double)HFE_TRACK_BLOCK_SIZE));
    for (size_t b = 0; b < track_block_count; b++) {
      memset(track_block, 0, sizeof(track_block));
      for (size_t h = 0; ((h < number_of_head) && (h < 2)); h++) {
        size_t left_track_byte_size = track_sizes[h] - (b * HFE_TRACK_BLOCK_SIDE_SIZE);
        size_t block_byte_size = ((HFE_TRACK_BLOCK_SIDE_SIZE <= left_track_byte_size) ? HFE_TRACK_BLOCK_SIDE_SIZE : left_track_byte_size);
        memcpy(track_block + (h * HFE_TRACK_BLOCK_SIDE_SIZE), track_bytes[h] + (b * HFE_TRACK_BLOCK_SIDE_SIZE), block_byte_size);
      }
